<script>
	...
	export let navBreakout = false;
	export let navTitleIsSame = false;
	export let navTitle = "";
	export let title = "";
...
<div
	class="content {cn(className)}"
	in:fly={{ duration: 900, y: 250, delay: 250 }}
	out:fly={{ duration: 250, y: 250 }}
>
	<Nav
		title={navTitleIsSame ? title : navTitle}
		class={cn(navClass)}
		{headerClass}
		breakout={navBreakout}
	/>
	<slot />
	<Footer />
</div>
