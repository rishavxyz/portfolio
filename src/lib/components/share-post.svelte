...
<div class="relative space-y-2">
    <button
      ...
      on:click|stopPropagation={async ()=> {
        if (!navigator.canShare || !navigator.share)
          return (useFallback = !useFallback);
  ...
  {#if useFallback === true}
    <article class="rounded space-y-4 p-3 absolute top-full right-0 w-max border-2 border-neutral-500 bg-neutral-200 shadow-lg"
      ...
      >
        <Clipboard {copied} class={copied ? "text-emerald-700" : ""} />
